...
7. 文物没有name属性，在匹配时不要用a:Artifact {{name:}}，要用a:Artifact {{Title:}}
8. 当用户查询某朝代/某材质/某博物馆等有哪些文物（即列出文物）时，只需返回 a.Title AS artifact_name
9. 当用户查询某文物的详细信息（如某文物的朝代、材质等）时，返回相应字段，如：a.Title AS artifact_name, Dynasty.name AS dynasty_info
...
def refine_cypher(raw_cypher: str) -> str:
    ...
    # 如果有多个 MATCH-RETURN 块，只保留最后一个
    if "MATCH" in cypher and "RETURN" in cypher:
        query_blocks = re.findall(r"MATCH.*?RETURN.*?(?=MATCH|$)", cypher, re.DOTALL)
        if query_blocks:
            cypher = query_blocks[-1].strip()
